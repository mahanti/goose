  ...
  ({ session, isOpen, onClose, onSave, disabled = false }) => {
    ...
    const handleSave = useCallback(async () => {
      ...
      try {
        ...
        onClose();
        setTimeout(() => {
          toastSuccess({ title: 'Success', msg: 'Session description updated successfully' });
        }, 300);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        console.error('Failed to update session description:', errorMessage);
        toastError({ title: 'Error', msg: `Failed to update session description: ${errorMessage}` });
  ...
  ({ onSelectSession, selectedSessionId }) => {
    ...
    const handleConfirmDelete = useCallback(async () => {
      ...
      try {
        ...
        toastSuccess({ title: 'Success', msg: 'Session deleted successfully' });
        loadSessions();
      } catch (error) {
        console.error('Error deleting session:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        toastError({ title: 'Error', msg: `Failed to delete session "${sessionName}": ${errorMessage}` });
      }
