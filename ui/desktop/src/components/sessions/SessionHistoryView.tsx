...
const SessionHistoryView: React.FC<SessionHistoryViewProps> = ({
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      toastError({
        title: 'Error',
        msg: `Failed to share session: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    } finally {
      setIsSharing(false);
    }
  ...
  const handleCopyLink = () => {
    navigator.clipboard
      ...
      .catch((err) => {
        ...
        toastError({ title: 'Error', msg: 'Failed to copy link to clipboard' });
      });
  };
  ...
  const handleLaunchInNewWindow = () => {
    try {
      ...
      toastError({ title: 'Error', msg: `Could not launch session: ${error instanceof Error ? error.message : error}` });
    }
  };
