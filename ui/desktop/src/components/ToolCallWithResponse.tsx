...
export default function ToolCallWithResponse({
  ...
  // Convert current data structure to ToolPart format for prompt-kit Tool component
  const getToolState = (): 'pending' | 'running' | 'completed' | 'error' => {
    if (isCancelledMessage) return 'error';
    if (isStreamingMessage && !toolResponse) return 'running';
    if (!toolResponse) return 'pending';
    
    const resultStatus = toolResponse.toolResult?.status;
  ...
  const toolPart = {
    type: toolCall.name.substring(toolCall.name.lastIndexOf('__') + 2) || toolCall.name,
    state: getToolState(),
    input: toolCall.arguments as Record<string, unknown>,
    output: toolResponse?.toolResult?.value ? {
      result: toolResponse.toolResult.value
    } : undefined,
    toolCallId: toolCall.id,
    errorText: toolResponse?.toolResult?.status === 'error' 
      ? (toolResponse.toolResult.error as string) 
      : undefined,
  };
          ...
          if (isUIResource(content)) {
            return (
              <div key={`${content.type}-${index}`} className="mt-3">
                ...
                <div className="mt-3 p-4 py-3 border border-borderSubtle rounded-xl bg-background-muted flex items-center">
...
function ToolCallExpandable({
  ...
  return (
    <div className="relative flex items-start">
      <div className={`bg-gray-100 dark:bg-gray-700 rounded-3xl border-0 overflow-hidden w-fit ${className}`}>
        <Button
          onClick={toggleExpand}
          className="group w-full flex justify-between items-center pr-2 transition-colors rounded-3xl hover:bg-gray-200 dark:hover:bg-gray-600"
          variant="ghost"
        >
        ...
        {isExpanded && <div className="rounded-b-3xl overflow-hidden">{children}</div>}
      </div>
      
      {/* Spacer to match ellipsis button width */}
      <div className="ml-2 mt-1 p-1 w-4 h-4"></div>
    </div>
  );
}
