...
export default function ExtensionItem({
  ...
  return (
    <Card
      className="transition-all duration-200 hover:cursor-pointer overflow-hidden"
      onClick={() => handleToggle(extension)}
    >
      <CardHeader>
        <div className="flex items-center gap-3">
          <ExtensionLogo extension={extension} size={40} />
          <CardTitle className="flex-1 min-w-0">{getFriendlyTitle(extension)}</CardTitle>
        </div>
